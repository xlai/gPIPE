...
bind_rows(updatedModel, .id = "Dose Levels") %>% 
...
```{r, echo = TRUE}
# Get names from the drug combination levels
relevant_names <- names(drugcombi_new$getDoseCombinationsLevel())
# Re-order the updatedModel_posterior list
relevant_posterior <- updatedModel_posterior[relevant_names]
...
dose_configs_valid <- purrr::keep(dose_configs_new, ~ any(.x$isValid ==TRUE))
```
The PIPE estimator is initialised with $weight = 1$ and $ \epsilon = 0.05$.
```{r, echo = TRUE}
pipe_hat <- PipeEstimator(dose_configs_valid, epsilon = 0.05)
```
...
```{r, echo = TRUE}
for (i in seq(1, cohort_size)){
    patientDataModel$addPatientData(doseCombination = current_dose_level, outcome = 0)
}
```
