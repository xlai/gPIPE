...
#        expand.grid()
#all_configs <-  lapply(seq_len(nrow(all_configs)), function(i) as.numeric(all_configs[i,]))
all_configs <- monotonic.matrices(6,6)
all_configs <- lapply(all_configs, function(x)c(x))
dose_configs_new <-lapply(all_configs,
    function(x) createDoseConfig(drugCombiObject = drugcombi_new, isNonDecreasing = TRUE, currentConfig = as.numeric(x))
)
dose_configs_valid <- purrr::keep(dose_configs_new, ~ any(.x$isValid ==TRUE))
```
...
```{r, echo = TRUE}
pipe_hat <- PipeEstimator(dose_configs_valid, epsilon = 0.5)
```
...
source('./src/strategies/admissibleRules/Adjacent.R')
source('./src/strategies/admissibleRules/Closest.R')
source('./src/strategies/admissibleRules/Neighbour.R')
source('./src/strategies/admissibleRules/Safety.R')
...
selection_rule_list <- list(
    rule_smallest_ss <- SelectionStrategy$new()
)
```
...
prob_true <- read.csv('tests/dose_levels_probability.csv')
...
runTrialSimulation('dose1.dose1', 2, 20, prob_true_list, drugCombinationModel, drugcombi_new, pipe_hat, admissible_rule_list, selection_rule_list)